...
export default function ClassCard({ classItem }: ClassCardProps) {
  ...
  const statusColors = {
    ...
    "on going": "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300",
  }

  const typeColors = {
    private: "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300",
    "no private": "bg-zinc-100 text-zinc-800 dark:bg-zinc-800/50 dark:text-zinc-300",
  }
    ...
    <motion.div whileHover={{ y: -5 }} transition={{ type: "spring", stiffness: 300 }}>
      <Card
        className={`overflow-hidden transition-all duration-300 border-zinc-200/60 dark:border-zinc-800/60 h-full flex flex-col ${
          isHovered ? "shadow-lg transform border-zinc-300 dark:border-zinc-700" : "shadow-sm hover:shadow-md"
        }`}
      ...
      >
        <div className="relative h-52 w-full overflow-hidden">
          ...
          <div className="absolute top-3 right-3 flex gap-2">
            <Badge variant="secondary" className={statusColors[classItem.status]}>
        ...
        <CardHeader>
          <div className="flex justify-between items-start">
            <div>
              <CardTitle className="text-xl text-zinc-900 dark:text-zinc-50">{classItem.name}</CardTitle>
              <CardDescription className="text-zinc-500 dark:text-zinc-400 mt-1">{classItem.category}</CardDescription>
            </div>
            <Badge
              variant="outline"
        ...
        <CardContent className="flex-grow">
          <p className="text-zinc-700 dark:text-zinc-300 mb-4 line-clamp-2">{classItem.description}</p>
